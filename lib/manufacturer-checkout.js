...
/** MT promotions/get returns rep-group promos — keep only this manufacturer's rows. */
export function filterPromotionsForManufacturer(promotions, manufacturerID) {
  const id = String(manufacturerID ?? '');
  if (!id) return [];

  return parsePromotionList(promotions).filter(
    (promo) => String(promo.manufacturerID ?? promo.manufacturerId ?? '') === id
  );
}

export function normalizePromotionsForManufacturer(response, manufacturerID) {
  return filterPromotionsForManufacturer(response, manufacturerID)
